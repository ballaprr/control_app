...
if pre_game_index is not None:
    # Find the "CLOCK" and "COURTSIDE" columns in the row immediately after PRE-GAME
    clock_column = None
    ...
    for idx in range(pre_game_index + 2, len(df)):
        ...
        if clock_column is not None and pd.notna(row.iloc[clock_column]):
            ...
            # Add to dictionary, ensuring COURTSIDE values are stored as lists
            if clock_value not in pre_game_data["PRE_GAME"]:
                pre_game_data["PRE_GAME"][clock_value] = []
            pre_game_data["PRE_GAME"][clock_value].append(courtside_value)
